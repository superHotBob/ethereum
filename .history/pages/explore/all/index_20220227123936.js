...
export default function ExploreAll() {
  ...
  useEffect(() => {
    ...
    async function tokensList() {
      ...
      const new_list = list.map((i,index)=>i.tokenId);
    ...
    async function ReadToken() {
      ...
      .then((res) => res);
      setImage(metadata => 
        [...metadata,{name:data.name,description:data.description,image:data.image}]
      );
    };
...
setTimeout(()=>console.log(metadata),2000);
  return (
    <div className="start_main">
      <h1>Explore ALL NFTs </h1> {tokenId.length}    
      {metadata.length > 0 &&        
