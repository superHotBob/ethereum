...
export default function ExploreAll() {
  ...
  useEffect(()=>{    
    async function ReadToken() {
      for ( const [index,i] of ids.entries() ) {
  ...
  return (
    <div className="start_main">
      <h1>Explore ALL NFTs </h1>{metadata.length}
      {metadata && <div className="all_nft" >

      {metadata.map((index,i) => <>
            <Link  href={`/token/${ids[index]}`}  passHref>
                        
              <div className="image_block" style={{ background: `url(${i.image})`}}>
                {/* <p className="bolls">
