...
export default function Token() {
  ...
  useEffect(()=>{ 
    console.log(asPath.replace('/token/',''));   
    async function ReadToken() {
      const my_owner = await contract.methods.ownerOf(asPath.replace('/token/','')).call();
      setOwner(my_owner);
      const owner = await contract.methods.tokenURI(asPath.replace('/token/','')).call();
