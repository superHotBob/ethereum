...
export default function Token() {
  ...
  useEffect(()=> console.log('HREF',window.location.href.lastIndexOf('/')),[]);
  ...
  useEffect(()=>{ 
    let path = asPath.replace('/token/','');
    async function ReadToken() {
      const my_owner = await contract.methods.ownerOf(path).call();
      setOwner(my_owner);
      const owner = await contract.methods.tokenURI(path).call();
